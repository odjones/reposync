...
if [ "$?" == 0 ]; then
    ...
    reposync --config=${REPOCFG} --gpgcheck --plugins --source --downloadcomps --download-metadata --download_path=${BASEDIR}/
        ...
        for REPO in *; do
            ...
            if [ "${CLEAN}" == 1 ]; then
                ...
                if [ -f ${BASEDIR}/${REPO}/comps.xml ]; then
                    ...
                    createrepo ${BASEDIR}/${REPO} --groupfile comps.xml

                else

                    # Create repository information from scratch
                    createrepo ${BASEDIR}/${REPO} >/dev/null
                ...
                if [ -f ${BASEDIR}/${REPO}/comps.xml ]; then
                    ...
                    createrepo ${BASEDIR}/${REPO} --groupfile comps.xml --update

                else

                    # Update repository information
                    createrepo ${BASEDIR}/${REPO} --update >/dev/null
